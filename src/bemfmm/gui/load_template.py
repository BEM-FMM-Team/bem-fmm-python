...
def load_template(name):
    ...
    def extract_string(mat):
        return (
            np.asarray(mat["strcoil"]["Pwire"].item(), dtype=float),
            np.asarray(mat["strcoil"]["Ewire"].item(), dtype=int),
            np.asarray(mat["strcoil"]["Swire"].item(), dtype=float).reshape(-1, 1),
        )
    ...
    template.cad_template_P = template.cad_template_P - np.sum(
        template.cad_template_P, axis=0
    ) / len(template.cad_template_P)
    template.str_template_P = template.str_template_P - np.sum(
        template.str_template_P, axis=0
    ) / len(template.str_template_P)
