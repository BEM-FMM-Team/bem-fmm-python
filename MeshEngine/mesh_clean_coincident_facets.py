...
def mesh_clean_coincident_facets(P, t, normals, centroids, areas, Indicator, condin, condout, contrast, accuracy):
...
#   Copyright WAW/SNM 2019-2024
    ...
    index_trimmed = np.sort(index_trimmed, axis = 1)[:, ::-1]   #Higher index first in each row
    index_trimmed = index_trimmed[np.lexsort((index_trimmed[:, 1], index_trimmed[:, 0]))]   #Order rows lowest to highest
    ...
    coincidentFacets = -np.ones((facetList1.shape[0], 2), dtype = int) #python begins indexing at 0, so 0 may not be used as a null value. Instead, we will use -1.
    ...
    for j in range (facetList1.shape[0]):
        ...
        vertices1 = vertices1[np.lexsort((vertices1[:,2], vertices1[:,1], vertices1[:,0]))]
        vertices2 = vertices2[np.lexsort((vertices2[:,2], vertices2[:,1], vertices2[:,0]))]
    ...
    coincidentFacets = coincidentFacets[
        coincidentFacets[:, 0] != -1
    ]
    coincidentCentroids = coincidentCentroids[
        coincidentCentroids[:, 0] != -1
    ]

    print(f" Found {coincidentFacetsCounter} duplicate facets")
    if(coincidentCentroidsCounter != 0):
        print(f"    Potential error: centroids coincide but the vertices do not. Correcting {len(coincidentCentroids)} facets")
        ...
        centroids[bad_idx, :] = (
            centroids[bad_idx, :]
            - eps * normals[bad_idx, :]) #dimensional mismatch in matlab
    else:
    ...
    if len(coincidentFacets) != 0:
        ...
        deleteFacet = np.maximum(coincidentFacets[:, 0], coincidentFacets[:, 1])      #    Delete facets of "previous" volumetric tissue (from the list, random)
        condout[keepFacet] = condin[deleteFacet]    #    Outer conductivity of surface becomes conductivity of volumetric tissue "previous" 
        condin[keepFacet] = condin[keepFacet]       #    Inner conductivity of surface remains conductivity of volumetric tissue "next"
        contrast[keepFacet] = ((condin[keepFacet] - condout[keepFacet]) / 
                            (condin[keepFacet] + condout[keepFacet])) # correct
        contrast[np.isnan(contrast)] = 0 #
        normals[keepFacet] = normals [keepFacet]    #    Normal vectors must be those for "next" volumetric tissue  

        #---Now, delete duplicated facets and all associated information---
        areas = np.delete(areas, deleteFacet, axis = 0) #   Areas of "previous" volumetric tissue are deleted
        centroids = np.delete(centroids, deleteFacet, axis = 0) #  Centroids of "previous" volumetric tissue are deleted
        Indicator = np.delete(Indicator, deleteFacet) #   Indicators of "previous" volumetric tissue are deleted
        normals = np.delete(normals, deleteFacet, axis = 0) #   Normals of "previous" volumetric tissue are deleted
        t = np.delete(t, deleteFacet, axis=0) #   Triangles of "previous" volumetric tissue are deleted
        condin = np.delete(condin, deleteFacet) #   All other infpormation for "previous" volumetric tissue are deleted
