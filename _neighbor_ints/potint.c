...
// define tolerance parameter 1e-10
#define TOL 0.0000000001

double dot (const double u_x, const double u_y, const double u_z,
	    const double v_x, const double v_y, const double v_z){
  return u_x * v_x + u_y * v_y + u_z * v_z;
}

double norm (const double u_x, const double u_y, const double u_z){
  return sqrt(dot(u_x,u_y,u_z,
		  u_x,u_y,u_z));
}

void cross(const double u_x, const double u_y, const double u_z,
	   const double v_x, const double v_y, const double v_z,
	   double* w_x, double* w_y, double* w_z) // output coords
{
  *w_x = u_y*v_z - u_z*v_y;
  *w_y = u_z*v_x - u_x*v_z;
  *w_z = u_x*v_y - u_y*v_x;
...
void single_layer_potint(const double r1_x, const double r1_y, const double r1_z,
			 const double r2_x, const double r2_y, const double r2_z,
			 const double r3_x, const double r3_y, const double r3_z,
			 const double n_x,  const double n_y,  const double n_z, // normal to triangle defined by r1,r2,r3
			 const double o_x,  const double  o_y, const double o_z, // observation point coords
			 double * I, double * Irho_x, double* Irho_y, double* Irho_z) // outputs I and Irho
{
  ...
  dx_21 = r2_x-r1_x; dy_21 = r2_y-r1_y; dz_21 = r2_z-r1_z;
  dx_31 = r3_x-r1_x; dy_31 = r3_y-r1_y; dz_31 = r3_z-r1_z;
  dx_32 = r3_x-r2_x; dy_32 = r3_y-r2_y; dz_32 = r3_z-r2_z;
  ...
  cross(l1_x, l1_y, l1_z,
	n_x,  n_y,  n_z,
	&u1_x, &u1_y, &u1_z); // u_1 = cross(l1, n)
  
  cross(l2_x, l2_y, l2_z,
	-n_x, -n_y, -n_z,
	&u2_x, &u2_y, &u2_z); // u_2 = -cross(l2, n) = cross(l2,-n)

  cross(l3_x, l3_y, l3_z,
	n_x,  n_y,  n_z,
	&u3_x, &u3_y, &u3_z); // u_3 = cross(l1, n)

  // create projection vector of the observation point
  double ndot;
  ndot = dot(o_x, o_y, o_z,
	     n_x, n_y, n_z);
  ...
  double u[9] = {u1_x, u1_y, u1_z,
		 u2_x, u2_y, u2_z,
		 u3_x, u3_y, u3_z};
  double l[9] = {l1_x, l1_y, l1_z,
		 l2_x, l2_y, l2_z,
		 l3_x, l3_y, l3_z};
  ...
  *Irho_x = 0; *Irho_y = 0; *Irho_z = 0;
  for(int k = 0; k < 3; k++){
    c3  = 3*count; c3_1 = 3*(count+1);
    
    dpp_x = o_x - r[c3_1];
    dpp_y = o_y - r[c3_1+1];
    dpp_z = o_z - r[c3_1+2];
    distance_obs = fabs(dot(n_x, n_y, n_z,
			  dpp_x, dpp_y, dpp_z));
    ...
    // calculate l+ and l-
    lplus  = dot(l[3*k], l[3*k+1], l[3*k+2],
		 pplus_x - p_x, pplus_y - p_y, pplus_z - p_z);
    lminus = dot(l[3*k], l[3*k+1], l[3*k+2],
		 pminus_x - p_x, pminus_y - p_y, pminus_z - p_z);

    // perpendicular distance from vector to edge
    P0 = fabs(dot(u[3*k], u[3*k+1], u[3*k+2],
		  pminus_x - p_x,pminus_y - p_y, pminus_z - p_z));
    ...
    dotPHATu = dot(PHAT_x, PHAT_y, PHAT_z,
		   u[3*k], u[3*k+1], u[3*k+2]);
...
/*****************************************************************
 * gateway function callable by MATLAB                           *
 * prhs[0] - 1 x 3 triangle vertex                               *
 * prhs[1] - 1 x 3 triangle vertex                               *
 * prhs[2] - 1 x 3 triangle vertex                               *
 * prhs[3] - 1 x 3 triangle normal                               *
 * prhs[4] - N x 3 matrix of observation points                  *
 * ------------------------------------------------------------- *
 * plhs[0] - N x 1 vector of neighbor integrals I = Is(1/r)      *
 * plhs[1] - N x 3 matrix of neighbor integrals I = Is(vec(r)/r) *
 *****************************************************************/
void mexFunction(int nlhs, mxArray *plhs[],
		 int nrhs, const mxArray *prhs[]){
  
  // check number of inputs and outputs is correct
  if(nrhs != 5){
    mexErrMsgIdAndTxt("BEMFMM:potint:nrhs",
		      "Expected 5 inputs.");
  }
  if(nlhs != 2){
    mexErrMsgIdAndTxt("BEMFMM:potint:nlhs",
		      "Expected 2 outputs.");
  }
  ...
  if(!(mxGetN(prhs[0]) == 3 &&
       ...
       mxGetN(prhs[3]) == 3 &&
       mxGetN(prhs[4]) == 3)){
    mexErrMsgIdAndTxt("BEMFMM:potint4b:columDimensions",
		      "All inputs must have 3 columns.");
  }
  if(!(1 == mxGetM(prhs[0]) &&
       mxGetM(prhs[0]) == mxGetM(prhs[1]) &&
       mxGetM(prhs[1]) == mxGetM(prhs[2]) &&
       mxGetM(prhs[2]) == mxGetM(prhs[3]))){
    mexErrMsgIdAndTxt("BEMFMM:potint4b:rowDimensions",
		      "First four inputs must have exactly one row.");
  }
  ...
  if(!(mxIsDouble(prhs[0]) &&
       ...
       mxIsDouble(prhs[3]) &&
       mxIsDouble(prhs[4]))){
    mexErrMsgIdAndTxt("BEMFMM:potint4b:notDouble",
		      "All input matrices must be type double.");
  }

  // variable declarations
  double *r1,*r2,*r3,*normal, *obs;
  mwSize N;

  // pointer to data for output matrices
  double* I;    
  double* Irho;
  ...
  r2      = mxGetPr(prhs[1]);
  r3      = mxGetPr(prhs[2]);
  normal = mxGetPr(prhs[3]);
  obs     = mxGetPr(prhs[4]);

  N = mxGetM(prhs[4]); // number of observation points (rows of prhs[4])

  // create the output matrices
  plhs[0] = mxCreateDoubleMatrix(N,1,mxREAL);
  plhs[1] = mxCreateDoubleMatrix(N,3,mxREAL);
  
  // get pointers to the real data in the output matrix
  I       = mxGetPr(plhs[0]);
  Irho    = mxGetPr(plhs[1]);

  // perform the calculation in parallel
  // schedule(static) divides iterations into equal-sized chunks
  // this is good when all iterations take the same time (which is the case here)

  // mex wants mwSize (unsigned) for loops, but OpenMP wants signed integrals
  long k;
  long K = (long) N;
  
  //#pragma omp parallel for schedule(static)
  for (k = 0; k<K; k++){
    single_layer_potint(r1[0], r1[1], r1[2],
			r2[0], r2[1], r2[2],
			r3[0], r3[1], r3[2],
			normal[0], normal[1], normal[2],
			obs[k], obs[k+N], obs[k+2*N],
			&(I[k]),
			&(Irho[k]), &(Irho[k+N]), &(Irho[k+2*N]));
  }
}
