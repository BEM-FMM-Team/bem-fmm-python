...
class Renderer:
    def __init__(self, head_models, ViewPort, drag_callback):
        ...
        # self.plt.interactor.AddObserver("LeftButtonPressEvent",self.on_click,1.0)
        # self.plt.interactor.AddObserver("MouseMoveEvent",self.on_drag,1.0)
        # self.plt.interactor.AddObserver("LeftButtonReleaseEvent",self.on_release,1.0)
    ...
    def on_click(self, event):
        if self.dragging:
            self.dragging = False
        elif type(getattr(event.actor, "id", None)) is str:
            self.dragging_id = event.actor.id
            self.dragging = True
