...
class Frontend:
    ...
    def create_root(self):
        ...
        add_button4 = ttk.Button(self.root,text="Add Custom",command=self.import_custom_coil)
        add_button4.grid(row=6, column=2, columnspan=2, pady=10)

        #rotation flag
        self.auto_flag = tk.BooleanVar(value=True)
        auto_button = ttk.Checkbutton(self.root,text="Auto Orient",variable=self.auto_flag)
        ...
        add_button2 = ttk.Button(self.root,text="Edit Coil",command=self.edit_selected_coil)
        add_button2.grid(row=7, column=1, columnspan=2, pady=10)

        add_button3 = ttk.Button(self.root,text="Delete Coil",command=self.delete_selected_coil)
        add_button3.grid(row=7, column=2, columnspan=2, pady=10)

        #computing Efield
        ttk.Label(self.root,text="Electric Field:").grid(row=8,column=0,padx=5,pady=5)

        add_button5 = ttk.Button(self.root,text="save",command=self.backend.save_coil_config)
    ...
    def add_coil(self):
        ...
        self.backend.new_coil(np.array([x, y, z]),coil_type,dIdt,auto,[self.root.winfo_x(),self.root.winfo_y()])
    ...
    def edit_selected_coil(self):
        ...
        x1 = tk.Scale(editor,from_=-.15,to=.15,resolution=.0001,orient="horizontal",variable=x_var,length=200)
        ...
        y1 = tk.Scale(editor,from_=-.15,to=.15,resolution=.0001,orient="horizontal",variable=y_var,length=200)
        ...
        z1 = tk.Scale(editor,from_=-.15,to=.15,resolution=.0001,orient="horizontal",variable=z_var,length=200)
        ...
        rx1 = tk.Scale(editor,from_=-180.0,to=180.0,resolution=.001,orient="horizontal",variable=rx_var,length=200)
        ...
        ry1 = tk.Scale(editor,from_=-180,to=180,resolution=.001,orient="horizontal",variable=ry_var,length=200)
        ...
        rz1 = tk.Scale(editor,from_=-180,to=180,resolution=.001,orient="horizontal",variable=rz_var,length=200)
        ...
        x_var.trace_add("write", lambda *args:self.backend.edit_coil_com(id, np.array([x_var.get(), y_var.get(), z_var.get()])))
        y_var.trace_add("write", lambda *args:self.backend.edit_coil_com(id, np.array([x_var.get(), y_var.get(), z_var.get()])))
        z_var.trace_add("write", lambda *args:self.backend.edit_coil_com(id, np.array([x_var.get(), y_var.get(), z_var.get()])))
        ...
        i_var.trace_add("write", lambda *args:self.backend.edit_coil_cur(id, i_var.get()))
        ...
        twist_bar = tk.Scale(editor,from_=-180,to=180,resolution=.001,orient="horizontal",variable=twist,length=200)
