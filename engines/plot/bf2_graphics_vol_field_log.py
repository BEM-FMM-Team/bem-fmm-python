...
def bemf2_graphics_vol_field_log(
    temp=None, th1=None, th2=None, levels=None, a=None, b=None
):
    ...
    cb.Ticks = np.diff(cbscalein) * (ticks - cbscaleout(1)) / np.diff(
        cbscaleout
    ) + cbscalein(1)
    origvalues = np.multiply(
        scale * np.sign(cb.Ticks),
        (10.0 ** (np.multiply(cb.Ticks, np.sign(cb.Ticks))) - 1),
    )
