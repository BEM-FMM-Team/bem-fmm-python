...
def bemf5_volume_field_electric(
    ...
    ineighborlocal = rangesearch(
        Points, Center[eligibleTriangles, :], R * Size, "NSMethod", "kdtree"
    )  # INFO
    ...
    for j in np.arange(1, len(eligibleTriangles) + 1).reshape(-1):
        ...
        if not len(index) == 0:
            temp = np.repmat(Center[m, :], (len(index), 1) - Points[index, :])
            DIST = np.sqrt(dot(temp, temp, 1))
            I = Area[m] * temp / np.repmat(DIST**3, (1, 3))
