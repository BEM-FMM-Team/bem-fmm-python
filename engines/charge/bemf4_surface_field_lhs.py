...
def bemf4_surface_field_lhs(
    ...
    weight_correction = weight * (np.sum(c.reshape((-1, 1)) * area.reshape((-1, 1))) / np.sum(area, 0))
    ...
    dominant_part = 2 * correction # This is the dominant (exact) matrix part and the "undo" terms for center-point FMM
    LHS = (
        c
        - dominant_part   
        - not_dominant_center_point
        + weight_correction)
