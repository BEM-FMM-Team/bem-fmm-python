...
def potint(r1, r2, r3, normal, ObsPoint):
    ...
    IEEE Trans Antennas Propag 1984;AP-32 (3):276281 - uses the last
    ...
    normabsl1 = sqrt(np.sum((r2 - r1) ** 2))
    normabsl2 = sqrt(np.sum((r3 - r1) ** 2))
    normabsl3 = sqrt(np.sum((r3 - r2) ** 2))
    temp0 = np.hstack(
    ...
    u = np.zeros(9)
    u[0:3] = +np.cross((r2 - r1) / normabsl1, normal)
    u[3:6] = -np.cross((r3 - r1) / normabsl2, normal)
    u[6:9] = +np.cross((r3 - r2) / normabsl3, normal)
    u = np.tile(u, (N, 1))

    # Create projection vector of the observation point
    NORM = np.tile(normal, (N, 1))
    ndot = np.sum(ObsPoint * NORM, axis=1)
    p = ObsPoint - np.tile(ndot, (3, 1)).T * NORM

    # Calculation of the analytic formula
    count = 0
    for c1 in range(3):
        # Distance of observation point perpendicular to the plane with triangle
        temporary = ObsPoint - r[:, slice(3 * count + 1, 3 * count + 4)]
        distanceobs = np.abs(np.sum(NORM * temporary, axis=1))

        # Calculate p+ and p-
        d1 = np.sum(NORM * r[:, slice(3 * count, 3 * count + 3)], axis=1)
        d2 = np.sum(NORM * r[:, slice(3 * count + 3, 3 * count + 6)], axis=1)

        pplus = r[:, slice(3 * count, 3 * count + 3)] - np.tile(d1, (3, 1)).T * NORM
        pminus = (
            r[:, slice(3 * count + 3, 3 * count + 6)] - np.tile(d2, (3, 1)).T * NORM
        )

        # Calculate l+ and l-
        lplus = np.sum(l[:, slice(3 * c1, 3 * c1 + 3)] * (pplus - p), axis=1)
        lminus = np.sum(l[:, slice(3 * c1, 3 * c1 + 3)] * (pminus - p), axis=1)

        # Perpendicular distance from projection vector to edge
        P0 = np.abs(np.sum(u[:, slice(3 * c1, 3 * c1 + 3)] * (pminus - p), axis=1))
        ...
        PHAT = (
            pminus - p - np.tile(lminus, (3, 1)).T * l[:, slice(3 * c1, 3 * c1 + 3)]
        ) / np.tile(P0, (3, 1)).T
        ...
        d1 = P0 * np.log((RPLUS + lplus) / (RMINUS + lminus))
        d2 = distanceobs * (np.arctan((P0 * lplus) / (R0 * R0 + distanceobs * RPLUS)))
        d3 = distanceobs * (np.arctan((P0 * lminus) / (R0 * R0 + distanceobs * RMINUS)))

        dotPHATu = np.sum(PHAT * u[:, slice(3 * c1, 3 * c1 + 3)], axis=1)
        S[:, c1] = (d1 - d2 + d3) * dotPHATu

        # A value of one term of the analytic sum RHO /R
        d1 = R0 * R0 * np.log((RPLUS + lplus) / (RMINUS + lminus))
        d2 = lplus * RPLUS - lminus * RMINUS
        d3 = d1 + d2
        Z[:, slice(3 * c1, 3 * c1 + 3)] = (
            np.tile(d3, (3, 1)).T * u[:, slice(3 * c1, 3 * c1 + 3)]
        )
