...
def mesh_neighborints_Pn(P, t, normals, Area, Center, RnumberP, ineighborP, contrast, numThreads):
...
#   Copyright SNM 2018-2022
    ...
    N = t.shape[0]
    integralpe = np.zeros((RnumberP, N))    #   exact potential integrals for array of neighbor triangles 
    integralpc = np.zeros((RnumberP, N))    #   center-point potential integrals for array of neighbor triangles 
    ...
    for n in range(M):  #needs to be reexamined   %   inner integral; (n =1 - first column of the system matrix, etc.)   
        r1 = P[t[n, 0], :]      #   [1x3]    
        r2 = P[t[n, 1], :]      #   [1x3]  
        r3 = P[t[n, 2], :]      #   [1x3]                   
        ...
        for q in range(RnumberP):
            ...
            for p in range(IndexS):
                ObsPoints[p + q * IndexS, :] = coeffS[0, p] * P[t[num, 0]] + coeffS[1, p] * P[t[num, 1]] + coeffS[2, p] * P[t[num, 2]]
        JP, _ = potint(r1, r2, r3, normals[n, :], ObsPoints)        #   JP was calculated without the area Area(n)
        for q in range(RnumberP):
        ...
        temp = np.tile(Center[n, :], (RnumberP, 1)) - Center[index, :]  #   these are distances to the observation/target triangle
    ...
    PC = coo_matrix(
        ((const * (integralpe - integralpc)).ravel(), (ii.ravel(), jj.ravel())), shape = (N, N)   #   almost symmetric
    ).tocsr()
