...
def mesh_amr(
    c, P, t, normals, Area, Indicator, tissues, tissue_list, tissue_ignore, refinement
):
    ...
    for m in range(len(tissue_list)):
        percentage[m] = 0
        index_tissue = np.where(Indicator == m)[
            0
        ]  #   global indexes for all triangles in the object
        refine = np.intersect1d(
            index_refine, index_tissue
        )  #   global indexes of triangles to be refined for the m-th object
        refine = refine - len(
            np.where(Indicator < m)[0]
        )  #   local indexes of triangles to be refined for the m-th object   #
        #   Restore object
        obj = Mesh()
        obj.t = t[Indicator == m, :]
        ...
        if refine.size > 0:
            percentage[m] = (
                100 * len(refine) / len(index_tissue)
            )  #   normalized to the total number of facets in the tissue
            ref = Mesh()
    ...
    for m in range(N):
        ...
        ):  #   rearrange vertices to have exactly the outer normal
            t[m, [1, 2]] = t[m, [2, 1]]
    #   Process other data
