...
# DD - 6/25
    N = t.shape[0]
    integralxc = np.zeros((RnumberE, N))    #   center-point Ex integrals for array of neighbor triangle
    integralyc = np.zeros((RnumberE, N))    #   center-point Ey integrals for array of neighbor triangles
    integralzc = np.zeros((RnumberE, N))    #   center-point Ey integrals for array of neighbor triangles
    ...
    for n in range(N):  #   inner integral (n =1 - first column of the system matrix, etc.)  
        # Calculate observation points on this triangle
        ObsPoints = np.zeros((IndexS, 3))
        for p in range(IndexS):
            ObsPoints[p, :] = coeffS[0, p] * P[t[n, 0], :] + coeffS[1, p] * P[t[n, 1], :] + coeffS[2, p] * P[t[n, 2], :]
        ...
        temp    = np.tile(Center[n, :], (RnumberE, 1)) - Center[index, :] #   these are distances to the observation/target triangle
        DIST    = np.sqrt(np.sum(temp * temp, axis = 1))                             #   single column                
        I       = Area[n]*temp/(DIST[:, None]**3)                  #   center-point integral, standard format    
        I[0, :] = 0                                                    #   self integrals will give zero
        integralxc[:, n] = -I[:, 0]    #   center-point integrals, entries of non-zero rows of n-th column
        integralyc[:, n] = -I[:, 1]    #   center-point integrals, entries of non-zero rows of n-th column
        integralzc[:, n] = -I[:, 2]    #   center-point integrals, entries of non-zero rows of n-th column        
    ...
    const = 1/(4*np.pi)
    integralc = np.zeros((RnumberE, N)) #normal integral component for array of neighbor triangles (center point) - to speed up GMRES
    ...
    EC = coo_matrix(
        (
            const * (-integralc + integrale.T).ravel(),
            (ii.ravel(), jj.ravel())
    ),
    shape=(N, N)
).tocsr()
